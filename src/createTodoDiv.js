...
/** This function returns a HTML Div Element that's a Shortened Version of Todo
 * @constructor
...
export function createTodoDivShort(todo) {
    ...
    checkbox.addEventListener('click', (e) => {
        ...
        const titleDivOfCurrentListofTodos =
...
/** This function returns a HTML Div Element that's a Detailed Version of Todo
 * @constructor
 * @param {Object} todo - A Todo object
 */
export function createTodoDivDetailed(todo) {
    ...
    const todoDivDetailedName = createEle(
        'div',
        'class',
        'todoDivDetailedName'
    );
    const todoDivDetailedDescription = createEle(
        'div',
        'class',
        'todoDivDetailedDescription'
    );
    const todoDivDetailedDueDate = createEle(
        'div',
        'class',
        'todoDivDetailedDueDate'
    );
    const todoDivDetailedDuration = createEle(
        'div',
        'class',
        'todoDivDetailedDuration'
    );
    const todoDivDetailedPriority = createEle(
        'div',
        'class',
        'todoDivDetailedPriority'
    );

    const todoDivDetailsArr = [
        todoDivDetailedName,
        todoDivDetailedDescription,
        todoDivDetailedDueDate,
        todoDivDetailedDuration,
    ...
    todoDivDetailsArr.forEach((element) => {
        ...
        const camelCaseName =
            capitalizedName[0].toLowerCase() + capitalizedName.slice(1);
        ...
        saveEdit.addEventListener('click', (e) => {
            ...
            const todoObjKeyCamelCase =
                todoObjKey[0].toLowerCase() + todoObjKey.slice(1);
            const indexNum = countIndexNum(e);

            // save new todo value
            if (todoKey.innerText === 'Due Date') {
                const dateStringArr = todoValue.innerText.split('/');
                gyh.projects[projTitle].todoArray[indexNum]['dueDate'] =
                    new Date(
                        Number(dateStringArr[2]),
                        Number(dateStringArr[1]) - 1,
                        Number(dateStringArr[0])
                    );
            } else if (todoKey.innerText === 'Priority') {
                const selector = document.getElementById('editPriority');
                gyh.projects[projTitle].todoArray[indexNum]['priority'] =
                    selector.value;
                selector.nextSibling.classList.toggle('hidden');
                selector.remove();
            } else {
                gyh.projects[projTitle].todoArray[indexNum][
                    todoObjKeyCamelCase
                ] = todoValue.innerText;
            }
            ...
            todoDivShort.remove();
            const newTodoDivShort = createTodoDivShort(
                gyh.projects[projTitle].todoArray[indexNum]
            );
            newTodoDivShort.classList.add('hidden');
            todoDivDetailed.parentElement.insertBefore(
                newTodoDivShort,
                todoDivDetailed
            );

            // refreshes todoDivDetailed
            const newTodoDivDetailed = createTodoDivDetailed(
                gyh.projects[projTitle].todoArray[indexNum]
            );
            newTodoDivDetailed.classList.remove('hidden');
            todoDivDetailed.parentElement.insertBefore(
                newTodoDivDetailed,
                todoDivDetailed.nextSibling
            );
 ...
 * @constructor
 * @param {String} word - word that you would like to undo UpperCamelCase
 */
...
//
/** helper function to determine the index number of the todo
 * under a project based on click event of save edit button
 * @constructor
